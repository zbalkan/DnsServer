        ...
        public DnsBehavioralInput CreateBehavioralSnapshot(string clientIp)
        {
            lock (this)
            {
                ...
                return new DnsBehavioralInput
                {
                    ...
                    AvgTtl = _ttlValues.Any() ? (float)_ttlValues.Average() : 0,
                    AvgDomainEntropy = _entropyValues.Any() ? (float)_entropyValues.Average() : 0,
                    MaxDomainEntropy = _entropyValues.Any() ? (float)_entropyValues.Max() : 0,
                    ...
                    AvgRtt = _rttValues.Any() ? (float)_rttValues.Average() : 0,
                    ProtocolTcpRatio = (_queryCount > 0) ? (float)_tcpQueryCount / _queryCount : 0,
                    AvgUdpPayloadSize = _udpPayloadSizes.Any() ? (float)_udpPayloadSizes.Average() : 0,
                    DnssecOkRatio = (_queryCount > 0) ? (float)_dnssecOkCount / _queryCount : 0,
                    NumericRatio = _numericRatios.Any() ? (float)_numericRatios.Average() : 0,
                    NonAlphanumericRatio = _nonAlphanumericRatios.Any() ? (float)_nonAlphanumericRatios.Average() : 0,
                    AvgAnswerSize = _answerSizes.Any() ? (float)_answerSizes.Average() : 0,
